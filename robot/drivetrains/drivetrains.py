...
class DifferentialDrive:
    ...
    def __init__(self, livewindow, width, deadzone=0.1):
        self.left = ntproperty(livewindow + "/Left Motor Speed", 0, writeDefault=False)
        self.right = ntproperty(livewindow + "/Right Motor Speed", 0, writeDefault=False)
        self.width = width
    ...
    def vector(self) -> typing.Tuple[float, float]:
        l = _deadzone(self.left) * self.gearing[0] * self.motors[0]["free_speed"]
        r = _deadzone(self.right) * self.gearing[1] * self.motors[1]["free_speed"]

        forward = (l + r) * 0.5
        turn = (l - r) / float(self.width)
